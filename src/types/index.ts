...
export type IntentCategory =
  ...
  | 'generate_report'
  | 'map_relationships'
  | 'identify_missing_configuration'
...
export interface EndpointIntelligence {
  ...
  hydrationCandidates: string[];       // operationIds or paths that can hydrate
...
export interface GraphEdge {
  from: string;
  to: string;
  type: 'RETURNS' | 'REQUIRES' | 'HYDRATES' | 'LISTS' | 'SEARCHES' | 'MUTATES' | 'REFERENCES' |
        'HAS_PARENT' | 'HAS_CHILD' | 'USES_PROFILE' | 'BELONGS_TO';
  weight?: number;
  metadata?: Record<string, any>;
}
...
export interface EndpointGraph {
  ...
  hydrationMap: Map<string, string[]>; // objectRef -> possible hydrator opIds
}
...
export interface MissingInput {
  ...
  required: boolean;
  description: string;
  possibleSources: string[]; // e.g. 'user_request', 'search', 'parent_object', 'default'
}
...
export interface HydrationNode {
  ...
  objectType: string;
  currentPartial: any;
  targetHydrator: string; // operationId
...
export interface ResponseGraph {
  primary: any;
  objects: Map<string, any>; // key by id or path
  references: Array<{ path: string; value: any; typeHint?: string }>;
  collections: Array<{ path: string; items: any[] }>;
...
export interface CompletenessReport {
  ...
  blocked_fields: Array<{ field: string; reason: string }>;
}
...
export interface FullAnswer {
  ...
  source_chain: Array<{ step: string; operationId?: string; path?: string; status: number }>;
  hydration_status: {
