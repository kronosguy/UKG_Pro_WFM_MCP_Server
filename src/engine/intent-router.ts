...
export interface ParsedRequest {
  ...
  objectTypes: string[];
  entities: Record<string, string>; // id/name hints
  keywords: string[];
...
export class IntentRouter {
  ...
  parse(userRequest: string): ParsedRequest {
    ...
    if (/explain|why|how|detail|what is/.test(lower)) intent = 'explain';
    else if (/compare|difference|vs/.test(lower)) intent = 'compare';
    else if (/valid|check|correct/.test(lower)) intent = 'validate';
    else if (/trouble|error|issue|problem|fix/.test(lower)) intent = 'troubleshoot';
    ...
    for (const [phrase, type] of Object.entries(otMap)) {
      if (lower.includes(phrase)) objectTypes.push(type);
    }
    if (objectTypes.length === 0) objectTypes.push('Person'); // sensible default
    ...
    if (dateMatches.length >= 1) dateRange = { start: dateMatches[0], end: dateMatches[1] || dateMatches[0] };
  ...
  selectPrimaryEndpoints(parsed: ParsedRequest): EndpointIntelligence[] {
    ...
    for (const ot of parsed.objectTypes) {
      ...
      if (hyd && !selected.includes(hyd)) selected.push(hyd);
      const disc = this.endpoints.find(e => e.objectType === ot && e.classification === 'DISCOVERY_ONLY');
      if (disc && !selected.includes(disc)) selected.push(disc);
    }
    return selected;
  }
}
