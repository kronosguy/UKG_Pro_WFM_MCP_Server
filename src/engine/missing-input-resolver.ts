 ...
 */
import { MissingInput, DiscoveredCandidate, EndpointIntelligence } from '../types/index.js';
...
export class MissingInputResolver {
  constructor(
    ...
    private graph: EndpointGraphBuilder
  ) {}
  ...
  async resolve(missing: MissingInput[], context: any): Promise<{ resolved: Record<string, any>; clarification?: string }> {
    ...
    for (const m of missing) {
      ...
      if (!value && context.priorResults?.length) {
        for (const prior of context.priorResults) {
          if (prior?.[m.name]) { value = prior[m.name]; break; }
          if (prior?.data?.[m.name]) { value = prior.data[m.name]; break; }
        }
      }
  ...
  private async attemptDiscovery(idField: string, nameHint: string): Promise<string | null> {
