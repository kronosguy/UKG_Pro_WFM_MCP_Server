export class BaseUkgTool {
    ...
    async resolveMissingInputs(ctx) {
        ...
        if (missing.length === 0)
            return { resolved: ctx.resolvedInputs || {} };
        return this.resolver.resolve(missing, ctx);
    }
    async executeDiscovery(ctx) {
    ...
    async executePrimaryCall(ctx) {
        const intel = this.selectPrimaryEndpoint(ctx);
        if (!intel)
            throw new Error('No primary endpoint selected for ' + this.name);
        const res = await this.callEndpoint(intel, ctx);
        return { data: res.data, status: res.status, intel };
    }
    async hydrateResponseGraph(response, objectType, ctx) {
        const result = await this.hydrator.hydrate(response, objectType, ctx);
        return result;
    }
    async validateCompleteness(graph) {
        return this.validator.validate(graph);
    }
    async calculateConfidence(completeness, isWrite) {
        return this.scorer.score(completeness, true, isWrite, 0);
    }
    async formatAnswer(graph, confidence, completeness, trace, intel) {
    ...
    async writeAuditEvent(answer, ctx) {
        this.audit.log({
    ...
    async execute(userRequest, inputs = {}) {
        const ctx = {
            userRequest,
        ...
        if (discovery)
            ctx.priorResults.push(discovery);
        ...
        trace.push(...hydration.trace.map((t) => ({ step: 'hydrate', status: 200, note: t })));
    ...
    selectDiscoveryEndpoint(ctx) {
        const parsed = this.router.parse(ctx.userRequest);
        const cands = this.router.selectPrimaryEndpoints(parsed);
        return cands.find(c => c.classification === 'DISCOVERY_ONLY') || cands[0] || null;
    }
    async callEndpoint(intel, ctx) {
        let path = intel.path;
        const params = { ...ctx.resolvedInputs };
        // Fill path params
        for (const key of Object.keys(params)) {
            path = path.replace(`{${key}}`, encodeURIComponent(params[key]));
            if (path !== intel.path && key !== 'body')
                delete params[key]; // moved to path
        }
        if (intel.method === 'GET') {
    ...
    makeBlockedAnswer(req, question) {
        return {
