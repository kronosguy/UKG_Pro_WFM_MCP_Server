export class EndpointGraphBuilder {
    ...
    build(endpoints) {
        ...
        for (const ep of endpoints) {
            ...
            for (const ref of ep.referenceFields) {
                ...
                for (const hyd of ep.hydrationCandidates) {
                    ...
                    if (!this.graph.hydrationMap.has(key))
                        this.graph.hydrationMap.set(key, []);
                    this.graph.hydrationMap.get(key).push(hyd);
                }
            }
            // Index
            if (!this.graph.endpointsByDomain.has(ep.domain))
                this.graph.endpointsByDomain.set(ep.domain, []);
            this.graph.endpointsByDomain.get(ep.domain).push(ep.operationId);
            if (!this.graph.endpointsByObject.has(ep.objectType))
                this.graph.endpointsByObject.set(ep.objectType, []);
        ...
        for (const ep of endpoints) {
            for (const cand of ep.hydrationCandidates) {
                ...
                if (target) {
                    this.addEdge(`endpoint:${ep.operationId}`, `endpoint:${cand}`, 'HYDRATES', 0.8, { targetObject: target.objectType });
                }
            }
        }
        return this.graph;
    ...
    addEdge(from, to, type, weight = 1, metadata) {
    ...
    // Query helpers
    findHydratorsFor(objectType, refField) {
