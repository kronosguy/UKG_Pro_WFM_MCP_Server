export function classifyEndpoint(raw) {
    const method = (raw.method || 'GET');
    const path = raw.path || '';
    const opId = raw.operationId || '';
    const notes = raw.notes || '';
    let classification = raw.classification || 'DISCOVERY_ONLY';
    let riskLevel = raw.riskLevel || 'SAFE_READ';
...
function inferHydrationCandidates(opId, objectType) {
    ...
    if (objectType.toLowerCase().includes('schedule'))
        candidates.push('getScheduleDetail', 'getShiftDetail');
    if (objectType.toLowerCase().includes('timecard'))
        candidates.push('getTimecard', 'getTimecardPunches');
    if (objectType.toLowerCase().includes('punch'))
        candidates.push('getPunchDetail');
    if (objectType.toLowerCase().includes('known'))
        candidates.push('getKnownPlaceDetail');
